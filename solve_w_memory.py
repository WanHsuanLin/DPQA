...
class DPQA:
    ...
    def setArchitecture(self, bounds: Sequence[int]):
        ...
        self.n_x, self.n_y_e, self.n_c, self.n_r_e = bounds
        memory_region_row_num = 5
        self.n_y = self.n_y_e + memory_region_row_num
        self.n_r = self.n_r_e + memory_region_row_num
        ...
        for g in range(num_gate):
            for s in range(1, num_stage):  # since stage 0 is 'trash'
                if len(self.g_q[g]) == 2:
                    ...
                    (self.dpqa).add(Implies(t[g] == s, And(y[q0][s] == y[q1][s], y[q0][s] < self.n_y_e)))
                ...
                for q1 in range(q0+1, self.n_q):
                    for s in range(1, num_stage):
                        if self.gate_index[(q0, q1)] == -1:
                            (self.dpqa).add(
                                Or(x[q0][s] != x[q1][s], y[q0][s] != y[q1][s], y[q0][s] >= self.n_y_e))
                        else:
                            (self.dpqa).add(Implies
                                            (And(x[q0][s] == x[q1][s],
                                                 y[q0][s] == y[q1][s],
    ...
    #####################################
    # only for optimial mode, temporary use for preliminary exp data of memory architecture
    
    # constraint to encode if a qubit is affected by Rydberg laser
    def  constraints_rydberg_laser_effect(
            self,
            num_stage: int,
            t: Sequence[Any],
            y: Sequence[Any],
    ):
        rl = [[Bool(f"rl_q{q}_t{i}") for i in range(num_stage)]
            for q in range(self.n_q)]
        for i in range(self.n_q):
            for j in range(num_stage):
                (self.dpqa).add(rl[i][j] == And(y[i][j] < self.n_y_e, Or([(gate_t == j) for gate_t in t])))
        return rl

    def constraints_rydberg_laser_and_atom_transfer_count_pysat(
        self,
        bound: int,
        rl: Sequence[Any],
        at: Sequence[Any],
        ...
        from pysat.card import CardEnc
        numAt = len(at[0])*self.n_q
        numvar = len(rl[0])*self.n_q+numAt+1
        len_at = len(at[0])
        len_rl = len(rl[0])
        ancillary = {}
        # get the CNF encoding the cardinality constraint
        ...
        for conj in cnf:
            ...
            for i in conj:
                ...
                idx = val
                idx -= 1
                if i in range(1, numAt):
                    or_list.append(at[idx // len_at][idx % len_at])
                if i in range(numAt, numvar):
                    or_list.append(rl[(idx - numAt) // len_rl][(idx - numAt) % len_rl])
                elif i in range(-numAt+1, 0):
                    or_list.append(
                        Not(at[idx // len_at][idx % len_at]))
                elif i in range(-numvar+1, -numAt+1):
                    or_list.append(
                        Not(rl[(idx - numAt) // len_rl][(idx - numAt) % len_rl]))
                else:
                    if val not in ancillary.keys():
                        ancillary[val] = Bool("anx_{}".format(val+self.ancillary_num))
                    if i < 0:
    ...
    def check_rydberg_count(
        ...
        time_set = set()
        for gate_t in t:
            time_set.add(model[gate_t].as_long())
        
        n_rydberg = 0
        for at_list in at:
            for q_t_at in at_list:
                if is_true(model[q_t_at]):
                    n_rydberg += 1
        
        assert(len(time_set) * self.n_q == n_rydberg)
        return n_rydberg

    def check_atom_transfer_count(
            self,
            a: Sequence[Any],
    ):
        model = (self.dpqa).model()
        ...
        at_count = 0
        for a_qubit in a:
            for i in range(1, len(a_qubit)):
                if is_true(model[a_qubit[i-1]]) != is_true(model[a_qubit[i]]):
                    at_count += 1
        return at_count

    def constraints_atom_transfer(
            self,
            a: Sequence[Any],
    ):
    ...
    def solve_optimal(self, step: int):
        ...
        while not solved_batch_gates:
            ...
            t = self.constraint_gate_batch(step+1, c, r, x, y)
            rl = self.constraints_rydberg_laser_effect(step+1, t, y)
        ...
        ############
        n_rydberg = self.check_rydberg_count(t, rl) 
        n_at = self.check_atom_transfer_count(a)
        bound = n_rydberg + n_at
        print(f"Find a solution with n_at={n_at} and n_rydberg={n_rydberg}. (sum={bound})")
        bound -= 1
        
        t_s = time.time()
        # minimize atom tranfer
        reach_optimal_atom_transfer_count = (bound == -1)
        while not reach_optimal_atom_transfer_count:
            self.dpqa.push()
            
            self.constraints_rydberg_laser_and_atom_transfer_count_pysat(bound, rl, at)
            reach_optimal_atom_transfer_count =\
                True if (self.dpqa).check() == sat else False
            if reach_optimal_atom_transfer_count:
                n_at = self.check_atom_transfer_count(a)
                n_rydberg = self.check_rydberg_count(t, rl)
                bound = n_at + n_rydberg
                print(f"Find a solution with n_at={n_at} and n_rydberg={n_rydberg}. (sum={bound})")
                bound -= 1
            self.dpqa.pop()
